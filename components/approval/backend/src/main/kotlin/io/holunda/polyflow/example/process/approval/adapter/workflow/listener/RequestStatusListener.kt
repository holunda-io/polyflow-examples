...
class RequestStatusListener(
  ...
  fun notifyRequestStatusChange(task: DelegateTask) {
    ...
    when (task.taskDefinitionKey) {
      APPROVE_REQUEST -> {
        when (reader.getOptional(APPROVE_DECISION).orElseThrow { IllegalStateException("Approval decision must be provided") }) {
          APPROVE -> {
            notifyChange(
              request = request,
      ...
      AMEND_REQUEST -> {
        when (reader.getOptional(AMEND_ACTION).orElseThrow { IllegalStateException("Amend action must be provided") }) {
          RESUBMIT -> {
            notifyChange(
              request = request,
